...
output "cloud_network_interfaces" {
  ...
  value = {
    for interface in azurerm_network_interface.cenv :
...
output "resource_group_name" {
  ...
  value       = azurerm_resource_group.cenv.name
}

output "resources_location" {
  description = "The name of Azure region"
  value       = azurerm_resource_group.cenv.location
}

output "cloud_public_ips" {
  description = "Public IP addresses of VMs"
  value = {
    for public_ip in azurerm_public_ip.cenv :
    public_ip.name => public_ip.ip_address
  }
}
...
output "azure_keyvault_name" {
  ...
  value       = var.make_key_vault ? azurerm_key_vault.key_store[0].name : ""
}
